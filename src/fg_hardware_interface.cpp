...
hardware_interface::CallbackReturn FGHardwareInterface::on_configure(const rclcpp_lifecycle::State &)
{
    ...
    try
    {
        if (connection_type_ == "tcp" || connection_type_ == "TCP") {
            gripper_ = std::make_unique<FG>(onrobot_type_, ip_address_, port_);
        }
        /*
        else if (connection_type_ == "serial" || connection_type_ == "Serial") {
            // AÑADE ESTAS VARIABLES EN TU CLASE (si no existen)
            std::string serial_device = info_.hardware_parameters["serial_device"];
            int baudrate = std::stoi(info_.hardware_parameters["baudrate"]);

            gripper_ = std::make_unique<FG>(onrobot_type_, serial_device, baudrate);
        }
        */
